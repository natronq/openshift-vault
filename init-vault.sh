...
function vault_enable_kubernetes_auth {
  echo "Creating service account for Vault..."
  oc create sa vault-auth
  # Add policy to allow TokenReview requests from vault
  oc adm policy add-cluster-role-to-user system:auth-delegator system:serviceaccount:$namespace:vault-auth
  ...
  echo "Creating sample role with name 'backend' bound to namespace ${namespace}"
  vault write auth/kubernetes/role/backend bound_service_account_names=default bound_service_account_namespaces=$namespace policies=backend ttl=2h
